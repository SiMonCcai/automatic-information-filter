...
普通指纹去重只识别完全相同的条目。多个来源使用不同标题报道同一件事时，可以增加一个有状态的事件聚类 Processor：

1. 只对标题生成可替换的语义向量；
2. 在最近事件窗口中按余弦相似度匹配；
3. 命中后加入已有事件并复用稳定的输出 ID，不再创建第二条展示记录；
4. 保存全部成员、来源链接、相似度、评分和代表替换历史；
5. 所有成员先评分，只有明显高于当前代表的候选才执行摘要、分类等昂贵处理；
6. 向量失败时 fail open，作为独立事件继续处理。

模型、阈值、窗口、状态库和输出端都由适配器决定，核心框架不绑定具体实现。完整状态机、幂等更新和崩溃恢复建议见：[事件级去重设计](docs/event-deduplication.md)。
